...
export const commands: Chat.ChatCommands = {
	...
	authority(target, room, user, connection) {
		...
		for (const [id, symbol] of Users.globalAuth) {
			...
			rankLists[symbol].push(Server.nameColor(Users.globalAuth.usernames.get(id) || id));
		}
		...
		const buffer = Utils.sortBy(
			...
			([symbol, names]) => (
				`${(Config.groups[symbol] ? `<b>${Config.groups[symbol].name}s</b> (${symbol})` : symbol)}:<br/>` +
	...
	roomauth(target, room, user, connection, cmd) {
		...
		const buffer = Utils.sortBy(
			...
			return `${group}:\n` +
				Utils.sortBy(names).map(userid => {
					...
					return userid in targetRoom!.users && isOnline ? Server.nameColor(userid, true) : Server.nameColor(userid);
				}).join(', ');
		});
		...
		if (curRoom.settings.founder) {
			buffer.unshift(`${(curRoom.settings.founder ? `Room Founder:<br />${Users.get(curRoom.settings.founder) && Users.get(curRoom.settings.founder).connected ? Server.nameColor(curRoom.settings.founder, true) : Server.nameColor(curRoom.settings.founder)}` : ``)}<br />`);
		}
		if (targetRoom !== room) buffer.unshift(`${targetRoom.title} room auth:`);
		connection.send(`|popup||html|${buffer.join("<br />")}${userLookup}`);
	},
	roomauthhelp: [
